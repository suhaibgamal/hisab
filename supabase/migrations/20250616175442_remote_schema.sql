...
CREATE OR REPLACE FUNCTION public.set_test_user(user_auth_id uuid)
 RETURNS void
 LANGUAGE plpgsql
AS $function$
BEGIN
...
CREATE OR REPLACE FUNCTION public.create_group_with_manager(p_activity_log_privacy text, p_auto_approve_members boolean, p_description text, p_export_control text, p_group_name text, p_invite_code_visible boolean, p_member_limit integer, p_member_list_visibility text, p_password text, p_user_display_name text, p_user_id uuid)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
...
BEGIN
    ...
    INSERT INTO public.group_members (
        ...
        user_id,
        role,
        joined_at  -- Added this to match table structure
    )
    VALUES (
        v_group_id,
        p_user_id,
        'manager',
        CURRENT_TIMESTAMP  -- Set joined_at to current time
    );
