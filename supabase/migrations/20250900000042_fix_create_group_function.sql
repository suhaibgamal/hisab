...
DROP FUNCTION IF EXISTS public.create_group_with_manager(text, text, text, integer, boolean, text, text, text, uuid);
DROP FUNCTION IF EXISTS public.create_group_with_manager(text, text, text, integer, boolean, text);

-- Recreate the function with the correct parameters
...
BEGIN
  ...
  v_invite_code := replace(replace(replace(v_invite_code, '/', '_'), '+', '-'), '=', '');
